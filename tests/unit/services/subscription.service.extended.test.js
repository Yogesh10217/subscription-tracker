...
describe('SubscriptionService Extended Unit Tests', () => {
  ...
  test('updateSubscription should throw notFound if sub missing', async () => {
    jest.spyOn(subscriptionRepository, 'findById').mockResolvedValue(null);
    await expect(subscriptionService.updateSubscription('sub123', { price: 20 })).rejects.toThrow(
      ApiError
    );
  });
  ...
  test('updateSubscription should record PRICE_CHANGE and RENEWAL timeline events when fields change', async () => {
    ...
    await subscriptionService.updateSubscription(
      'sub123',
      { price: 20, renewalDate: new Date('2026-10-01') },
      'user123'
    );
    expect(timelineService.recordEvent).toHaveBeenCalled();
  });
  ...
  test('toggleFavorite and togglePin should modify flags or throw on mismatch', async () => {
    ...
    jest
      ...
      .mockResolvedValue({ ...mockSub, isFavorite: true });
  ...
  test('archiveSubscription and restoreSubscription should handle timeline events or throw', async () => {
    ...
    await expect(subscriptionService.archiveSubscription('sub123', 'user123')).rejects.toThrow(
      ApiError
    );

    jest.spyOn(subscriptionRepository, 'restore').mockResolvedValue(null);
    await expect(subscriptionService.restoreSubscription('sub123', 'user123')).rejects.toThrow(
      ApiError
    );
  });
  ...
  test('deleteSubscription soft delete vs hard delete', async () => {
    ...
    await expect(subscriptionService.deleteSubscription('sub123', 'user123')).rejects.toThrow(
      ApiError
    );
  ...
  test('cancelSubscription should mark status as Cancelled or throw if missing', async () => {
    jest
      .spyOn(subscriptionRepository, 'update')
      .mockResolvedValue({ ...mockSub, status: 'Cancelled' });
    jest.spyOn(timelineService, 'recordEvent').mockResolvedValue({});
    ...
    await expect(subscriptionService.cancelSubscription('sub123', 'user123')).rejects.toThrow(
      ApiError
    );
  });
  ...
  test('bulkOperation should handle archive, restore, delete, updateCategory, updateTags', async () => {
    ...
    jest
      .spyOn(subscriptionRepository, 'bulkUpdateCategory')
      .mockResolvedValue({ modifiedCount: 2 });
    jest.spyOn(subscriptionRepository, 'bulkUpdateTags').mockResolvedValue({ modifiedCount: 2 });

    await expect(subscriptionService.bulkOperation('invalid', ['id1'], 'user123')).rejects.toThrow(
      ApiError
    );
    await expect(subscriptionService.bulkOperation('archive', [], 'user123')).rejects.toThrow(
      ApiError
    );

    const archiveRes = await subscriptionService.bulkOperation(
      'archive',
      ['id1', 'id2'],
      'user123'
    );
    ...
    const restoreRes = await subscriptionService.bulkOperation(
      'restore',
      ['id1', 'id2'],
      'user123'
    );
    ...
    const categoryRes = await subscriptionService.bulkOperation(
      'updateCategory',
      ['id1', 'id2'],
      'user123',
      { categoryRef: 'cat1', categoryName: 'Streaming' }
    );
    expect(categoryRes.modifiedCount).toBe(2);

    const tagsRes = await subscriptionService.bulkOperation(
      'updateTags',
      ['id1', 'id2'],
      'user123',
      { tags: ['tag1'] }
